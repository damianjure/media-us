...
export function MatrixView() {
  return (
    <div className="p-4">
        ...
        <table className="w-full border-collapse min-w-[500px]">
          <thead>
            <tr>
              ...
              {dates.map((date) => (
                <th key={date} className="p-2 text-center text-xs font-medium text-slate-400 min-w-24">{date}</th>
              ))}
            </tr>
          </thead>
          <tbody>
                  ...
                  return (
                    <td key={date} className="p-1 border-t border-slate-100 dark:border-slate-800">
                      {person ? (
                        ...
                        <div className="border border-dashed border-slate-200 dark:border-slate-700 rounded-lg px-2 py-1.5 text-xs text-slate-300 dark:text-slate-600 text-center cursor-pointer hover:border-indigo-300 dark:hover:border-indigo-700">
                          Asignar
                        </div>
                      )}
                    </td>
      ...
      <p className="text-xs text-slate-400 mt-4 text-center">Tocá una celda para asignar o cambiar persona</p>
    </div>
  );
}