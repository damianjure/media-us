            ...
            >
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">
                  {team.members} miembros · {team.roles.length} roles
                </p>
                <div className="flex flex-wrap gap-1 mt-2">
                  {team.roles.map((role) => (
                    <span
                      key={role}
                      className="text-[10px] px-1.5 py-0.5 rounded-md bg-indigo-50 dark:bg-indigo-950 text-indigo-600 dark:text-indigo-400"
                    >
                      {role}
                    </span>
                  ))}
